...
class Auth extends CI_Controller
{
    ...
    public function forgot_password()
    {
        ...
        if ($this->config->item('identity', 'ion_auth') != 'email') {
            ...
            $this->form_validation->set_rules('identity', 'Email', 'required|xss_clean|valid_email');
            $this->form_validation->set_rules('mobile', 'Mobile', 'required|xss_clean|numeric|min_length[6]|max_length[15]');
        } else {
    ...
    public function register_user()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            if (isset($_POST['friends_code']) && !empty($_POST['friends_code'])) {
                ...
                $friends_cod = $_POST['friends_code'];
                $query = $this->db->select('*')
                    ->from('users')
                    ->where('referral_code', $friends_cod)
                    ->get();
                $result = $query->result();
                if (empty($result)) {
                    $response["error"]   = true;

...
class Products extends CI_Controller
{
    ...
    public function offers_and_flash_sale()
    {
        ...
        // fetch offers data
        $offer_slider = fetch_details('offer_sliders', '', '', '', '', 'row_order', 'ASC');
    ...
    public function get_details($product_id = '')
    {
        ...
        $valid_zipcode = (!empty($this->session->userdata('valid_zipcode'))) ? $this->session->userdata('valid_zipcode') : "";
        $product = fetch_product($user_id, null, $product_id, NULL, NULL, NULL, NULL, NULL, NULL, $valid_zipcode, 1);
        if (isset($product['product']) && empty($product['product'])) {
            return false;
        }
        $product['product'][0]['zipcode'] = $valid_zipcode;
        $product = $product['product'][0];
        $product['get_price'] = get_price_range_of_product($product['variants'][0]['special_price']);
    ...
    public function manage_offers()
    {
        ...
        if (isset($id) && !empty($id)) {
            ...
            // } else
            if (strtolower($offer_details[0]['type']) == "categories") {
            ...
            if (strtolower($offer_details[0]['type']) == "brand") {

                $brand_id = (!empty($offer_details[0]['type_id']) && isset($offer_details[0]['type_id'])) ? $offer_details[0]['type_id'] : '';
                ...
                $product  =  fetch_product('', (isset($filters)) ? $filters : null, '', '');
            }

...
class Custom_sms extends CI_Controller
{
    ...
    {
        if (null !== ($edit_sms = $this->input->post('edit_sms', true)) && !empty($edit_sms)) {
		...
		if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
			...
			if (!$this->form_validation->run()) {
                ...
                if (null !== $this->input->post('edit_custom_sms', true) && !empty($this->input->post('edit_custom_sms', true))) {
                    if (is_exist(['type' => $this->input->post('type', true)], 'custom_sms', $this->input->post('edit_custom_sms', true))) {
						$response["error"]   = true;
					...
					if (!$this->form_validation->is_unique($this->input->post('type', true), 'custom_sms.type')) {
						$response["error"]   = true;
						$response["message"] = "Name Already Exist ! Provide a unique type";
						$response['csrfName'] = $this->security->get_csrf_token_name();
						$response['csrfHash'] = $this->security->get_csrf_hash();
                ...
                $data = array(
                    'edit_custom_sms' => $this->input->post('edit_custom_sms', true),
                    'title' => $this->input->post('title', true),
                    'message' => $this->input->post('message', true),
                    'type' => $this->input->post('type', true),
                );

                $this->custom_sms_model->add_custom_sms($data);
                $this->response['error'] = false;
                $this->response['csrfName'] = $this->security->get_csrf_token_name();
                $this->response['csrfHash'] = $this->security->get_csrf_hash();
                $message = (null !== $this->input->post('edit_custom_sms', true)) ? 'SMS Updated Successfully' : 'SMS Added Successfully';

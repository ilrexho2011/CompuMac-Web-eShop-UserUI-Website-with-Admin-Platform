		...
		if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
			...
			if (!$this->form_validation->run()) {
				...
				if (isset($edit_id) && !empty($edit_id)) {
					if (is_exist(['mobile' => $this->input->post('mobile', true)], 'users', $this->input->post('edit_system_user', true))) {
						$this->response['error'] = true;
					...
					if (is_exist(['email' => $this->input->post('email', true)], 'users', $this->input->post('edit_system_user', true))) {
						$this->response['error'] = true;
						$this->response['message'] = 'Email is already registered. Please Provide Unique Email!';
						$this->response['csrfName'] = $this->security->get_csrf_token_name();
						$this->response['csrfHash'] = $this->security->get_csrf_hash();
						$this->response['data'] = array();
						print_r(json_encode($this->response));
					...
					if (is_exist(['mobile' => $this->input->post('mobile', true)], 'users')) {
						$this->response['error'] = true;
						$this->response['message'] = 'Mobile is already registered. Please Provide Unique Number !';
						$this->response['csrfName'] = $this->security->get_csrf_token_name();
						$this->response['csrfHash'] = $this->security->get_csrf_hash();
					...
					if (is_exist(['email' => $this->input->post('email', true)], 'users')) {
						$this->response['error'] = true;
						$this->response['message'] = 'Email is already registered. Please Provide Unique Email !';
						$this->response['csrfName'] = $this->security->get_csrf_token_name();
						$this->response['csrfHash'] = $this->security->get_csrf_hash();
				...
				$edit_system_user = $this->input->post('edit_system_user', true)  ;
                if (! empty($edit_system_user)) {
                    $data['edit_system_user'] = $this->input->post('edit_system_user', true)  ;
                }


				$this->system_users_model->update_user($data);
				$this->response['error'] = false;
				$this->response['csrfName'] = $this->security->get_csrf_token_name();
				$this->response['csrfHash'] = $this->security->get_csrf_hash();
				$message = (null != $edit_id) ? 'User Updated Successfully' : 'User Added Successfully';

...
class Featured_sections extends CI_Controller
{
    ...
    public function add_featured_section()
    {
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            $edit_featured_section = $this->input->post('edit_featured_section', true);
            if (isset($edit_featured_section)) {
            ...
            if (!$this->form_validation->run()) {
                ...
                if (isset($edit_featured_section)) {
                    ...
                    if (is_exist(['title' => $this->input->post('title', true)], 'sections', $this->input->post('edit_featured_section', true))) {
                        $response["error"]   = true;
                    ...
                    if (is_exist(['title' => $this->input->post('title', true)], 'sections')) {
                        $response["error"]   = true;
                        $response['csrfName'] = $this->security->get_csrf_token_name();
                        $response['csrfHash'] = $this->security->get_csrf_hash();
                        $response["message"] = "Title Already Exists !";
                ...
                $data = array(
                    ...
                    'digital_product_ids' => $this->input->post('digital_product_ids', true),
                );

                $edit_featured_section = $this->input->post('edit_featured_section', true);
                if (! empty($edit_featured_section)) {
                    $data['edit_featured_section'] = $this->input->post('edit_featured_section', true);
                }


                $this->Featured_section_model->add_featured_section($data);
                $this->response['error'] = false;
                $this->response['csrfName'] = $this->security->get_csrf_token_name();
                $this->response['csrfHash'] = $this->security->get_csrf_hash();
                $message = (null !== $this->input->post('edit_featured_section', true)) ? 'Section Updated Successfully' : 'Section Added Successfully';
    ...
    public function update_section_order()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (print_msg(!has_permissions('update', 'featured_section_order'), PERMISSION_ERROR_MSG, 'featured_section_order', false)) {
                return false;
            }
            $i = 0;
            $temp = array();

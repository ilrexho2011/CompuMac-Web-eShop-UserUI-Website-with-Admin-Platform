		...
		if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
			...
			if (!$this->form_validation->run()) {
				...
				$data = array(
					...
					'mail_content_type' => $this->input->post('mail_content_type', true),
					'smtp_encryption' => $this->input->post('smtp_encryption', true),
				);

				$this->Setting_model->update_email_settings($data);
				$this->response['error'] = false;
				$this->response['csrfName'] = $this->security->get_csrf_token_name();
				$this->response['csrfHash'] = $this->security->get_csrf_hash();
				$this->response['message'] = 'Email SMTP Updated Successfully';
				print_r(json_encode($this->response));
			}
		} else {
			redirect('admin/login', 'refresh');
